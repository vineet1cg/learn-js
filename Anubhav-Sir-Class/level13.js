...
function PanagramCheck(str) {
  ...
  return ascii >= 123 ? true : false;
}
console.log(PanagramCheck("the quick brown fox jumps over a lazy dog"));

// using includes

function PanagramCheckUsingIncludes(str) {
  str = str.toLowerCase();
  let checking = "abcdefghijklmnopqrstuvwxyz";
  for (let i = 0; i < str.length; i++) {
    if (!checking.includes(str[i])) {
      return false;
    }
  }
  return true;
...
console.log(
  PanagramCheckUsingIncludes("The quick brown fox jumps over a lazy dog")
);



// optimized approach ? => objects!
